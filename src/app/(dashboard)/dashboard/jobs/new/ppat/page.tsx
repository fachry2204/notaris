...
import { ArrowLeft, FilePlus2, Save, Landmark, User2, Clock, Calendar, Search, Plus, Trash2 } from "lucide-react";
...
export default function NewPpatPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) { toast.success("Berkas PPAT berhasil diregistrasi!"); router.push("/dashboard/jobs/inbound"); }
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground">Registrasi Berkas PPAT</h1>
            <p className="text-sm text-muted-foreground uppercase tracking-widest font-bold text-pink-500">Akta Jual Beli / Hibah / Balik Nama</p>
          </div>
        </div>
      </div>
      ...
      <Card className="border-none shadow-sm rounded-[2.5rem] bg-card overflow-hidden">
        <CardHeader className="border-b bg-muted/5 px-10 py-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <CardTitle className="text-xl">Formulir PPAT</CardTitle>
              <CardDescription>Masukkan detail pengurusan berkas pertanahan (PPAT).</CardDescription>
            </div>
          </div>
        </CardHeader>
            ...
            <div className="grid gap-x-10 gap-y-8 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <select className="flex h-11 w-full rounded-xl border border-muted bg-background px-4 font-medium" value={formData.pengurusanUntuk} onChange={(e) => setFormData(prev => ({ ...prev, pengurusanUntuk: e.target.value }))}>
                  ...
                  <option value="Sertifikat Hak Milik">Sertifikat Hak Milik (SHM)</option>
                  ...
                  {documents.map((doc, i) => (
                    <div key={doc.id} className="grid md:grid-cols-12 gap-4 items-end animate-in fade-in slide-in-from-left-2">
                      ...
                      <div className="md:col-span-6 space-y-2">
                        ...
                        <Input placeholder="Contoh: Sertifikat Asli, KTP Penjual, dll" value={doc.description} onChange={(e) => updateDocumentRow(doc.id, 'description', e.target.value)} className="h-11 rounded-xl" />
                      </div>
