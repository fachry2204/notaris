...
import { ArrowLeft, FilePlus2, Save, User2, Clock, Calendar, Search, Plus, Trash2 } from "lucide-react";
...
export default function NewNonBadanHukumPage() {
  const router = useRouter();
  const jobCategory = "Non Badan Hukum";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const jobData = {
      ...
      companyName: formData.namaNonBadanHukum,
    ...
    if (result.success) { toast.success("Berkas Non Badan Hukum berhasil diregistrasi!"); router.push("/dashboard/jobs/inbound"); }
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground">Registrasi Non Badan Hukum</h1>
            <p className="text-sm text-muted-foreground uppercase tracking-widest font-bold text-pink-500">Perorangan / Perjanjian / Kuasa</p>
          </div>
        </div>
      </div>
        ...
        <CardHeader className="border-b bg-muted/5 px-10 py-8">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 rounded-xl bg-pink-500/10 flex items-center justify-center">
              <User2 className="h-7 w-7 text-pink-500" />
            </div>
            <div>
              <CardTitle className="text-xl">Formulir Non Badan Hukum</CardTitle>
              <CardDescription>Masukkan detail pengurusan berkas perorangan atau perjanjian.</CardDescription>
            </div>
          </div>
        </CardHeader>
            ...
            <div className="grid gap-x-10 gap-y-8 md:grid-cols-2">
              ...
              <div className="md:col-span-2 space-y-6 pt-4">
                <div className="flex items-center justify-between border-b border-muted/20 pb-2">
                  <div className="flex items-center gap-2"><User2 className="h-5 w-5 text-pink-500" /><Label className="text-base font-bold">Informasi Pihak (Jika ada)</Label></div>
                  <Button type="button" variant="outline" size="sm" onClick={addPendiriRow} className="rounded-xl border-pink-200 text-pink-600 font-bold gap-2"><Plus className="h-4 w-4" />Tambah Pihak</Button>
                </div>
                <div className="space-y-4">
                  {pendiri.map((p, i) => (
                    <div key={p.id} className="p-6 rounded-3xl border border-muted bg-muted/5 relative">
                      <div className="absolute -top-3 left-6 px-3 py-0.5 bg-pink-500 text-white text-[10px] font-bold rounded-full uppercase">Pihak #{i + 1}</div>
                      ...
                      <div className="grid md:grid-cols-3 gap-6 pt-2">
                        <div className="space-y-2"><Label className="text-xs font-bold text-muted-foreground uppercase">Nama</Label><Input placeholder="Nama lengkap..." value={p.nama} onChange={(e) => updatePendiriRow(p.id, 'nama', e.target.value)} className="h-11 rounded-xl" required /></div>
                        <div className="space-y-2"><Label className="text-xs font-bold text-muted-foreground uppercase">No HP</Label><Input placeholder="628..." value={p.hp} onChange={(e) => updatePendiriRow(p.id, 'hp', e.target.value)} className="h-11 rounded-xl" /></div>
              ...
              <div className="space-y-2">
                <Label className="text-sm font-bold text-muted-foreground">Jenis Pekerjaan (Non Badan Hukum)</Label>
              ...
              <div className="space-y-2">
                <Label className="text-sm font-bold text-muted-foreground">Nama Entitas / Keterangan</Label>
                <Input placeholder="Contoh: Perjanjian Sewa Ruko Bpk. Agus" value={formData.namaNonBadanHukum} onChange={(e) => setFormData(prev => ({ ...prev, namaNonBadanHukum: e.target.value }))} className="h-11 rounded-xl border-muted px-4 font-bold" required />
              </div>
              ...
              <div className="space-y-2">
                ...
                <select className="flex h-11 w-full rounded-xl border border-muted bg-background px-4 font-medium" required value={formData.pengurusanUntuk} onChange={(e) => setFormData(prev => ({ ...prev, pengurusanUntuk: e.target.value }))}>
                  ...
                  <option value="Legalisasi Saja">Legalisasi Saja</option>
                </select>
              </div>
