...
import { ArrowLeft, Save, User2, Clock, Calendar, Search, Plus, Trash2, Upload } from "lucide-react";
...
export default function EditNonBadanHukumPage() {
  const { id } = useParams();
    ...
    const fetchData = async () => {
      ...
      if (jobRes.success) {
        ...
        setFormData({
          ...
          namaNonBadanHukum: job.companyName || "",
          namaSaksi: job.saksi || "",
          deadlineDays: job.deadline && job.createdAt ? Math.ceil((new Date(job.deadline).getTime() - new Date(job.createdAt).getTime()) / (1000 * 60 * 60 * 24)).toString() : "7",
          notes: job.notes || "",
          status: job.status,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) { toast.success("Berkas Non Badan Hukum diperbarui!"); router.push(`/dashboard/jobs/${id}`); }
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Edit Non Badan Hukum</h1>
      ...
      <Card className="border-none shadow-sm rounded-[2.5rem] bg-card overflow-hidden">
        <CardHeader className="border-b bg-muted/5 px-10 py-8">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 rounded-xl bg-pink-500/10 flex items-center justify-center text-pink-500"><User2 className="h-7 w-7" /></div>
            <div><CardTitle className="text-xl">Formulir Edit Non Badan Hukum</CardTitle><CardDescription>Update detail informasi perorangan atau perjanjian.</CardDescription></div>
          </div>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid gap-x-10 gap-y-8 md:grid-cols-2">
              ...
              <div className="md:col-span-2 space-y-6 pt-4">
                <div className="flex items-center justify-between border-b border-muted/20 pb-2"><div className="flex items-center gap-2 text-pink-500"><User2 className="h-5 w-5" /><Label className="text-base font-bold text-foreground">Informasi Pihak (Jika ada)</Label></div><Button type="button" variant="outline" size="sm" onClick={() => setPendiri([...pendiri, { id: Date.now(), nama: "", hp: "", email: "" }])} className="rounded-xl text-pink-600 font-bold"><Plus className="h-4 w-4 mr-2" />Tambah Pihak</Button></div>
              ...
              <div className="space-y-2">
                <Label className="text-sm font-bold text-muted-foreground">Jenis Pekerjaan</Label>
              ...
              <div className="space-y-2"><Label className="text-sm font-bold text-muted-foreground">Keterangan Entitas</Label><Input value={formData.namaNonBadanHukum} onChange={(e) => setFormData(prev => ({ ...prev, namaNonBadanHukum: e.target.value }))} className="h-11 rounded-xl border-muted px-4 font-bold" /></div>
