...
import { ArrowLeft, Save, FileText, User2, Clock, Calendar, Search, Plus, Trash2, Upload } from "lucide-react";
...
export default function EditUmumPage() {
  const { id } = useParams();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) { toast.success("Berkas diperbarui!"); router.push(`/dashboard/jobs/${id}`); }
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Edit Berkas</h1>
      ...
      <Card className="border-none shadow-sm rounded-[2.5rem] bg-card overflow-hidden">
        <CardHeader className="border-b bg-muted/5 px-10 py-8">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 rounded-xl bg-pink-500/10 flex items-center justify-center text-pink-500"><FileText className="h-7 w-7" /></div>
            <div><CardTitle className="text-xl">Formulir Edit Berkas</CardTitle><CardDescription>Update detail informasi pengerjaan berkas.</CardDescription></div>
          </div>
        </CardHeader>
        <CardContent className="p-10">
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid gap-x-10 gap-y-8 md:grid-cols-2">
              ...
              <div className="space-y-2"><Label className="text-sm font-bold text-muted-foreground">Nama Saksi</Label><Input value={formData.namaSaksi} onChange={(e) => setFormData({ ...formData, namaSaksi: e.target.value })} className="h-11 rounded-xl border-muted px-4 font-medium" /></div>
              ...
              <div className="space-y-2 md:col-span-2"><Label className="text-sm font-bold text-muted-foreground">Judul / Jenis Pekerjaan</Label><Input value={formData.jenisPekerjaan} onChange={(e) => setFormData({ ...formData, jenisPekerjaan: e.target.value })} className="h-11 rounded-xl border-muted px-4 font-bold" /></div>
