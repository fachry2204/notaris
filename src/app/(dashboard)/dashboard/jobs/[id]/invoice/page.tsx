...
import { createInvoice, getInvoiceByJobId } from "@/lib/actions/invoice";
...
export default function CreateInvoicePage() {
  ...
  useEffect(() => {
    const fetchJobAndInvoice = async () => {
      ...
      const invoiceResult = await getInvoiceByJobId(id as string);
      if (invoiceResult.success && invoiceResult.data) {
        toast.info("Invoice sudah dibuat untuk berkas ini.");
        router.replace(`/dashboard/invoice/${invoiceResult.data.id}`);
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let jobType: "badan_hukum" | "non_badan_hukum" | "ppat" = "badan_hukum";
    if (job.category === "Non Badan Hukum") jobType = "non_badan_hukum";
    if (job.category === "PPAT") jobType = "ppat";
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        {job?.id && (
          <Button 
            ...
            onClick={() => window.open('/print/invoice/' + job.id, '_blank')}
          >
